<div align="center">

<img src="https://avatars.githubusercontent.com/u/45946096?v=4" width="150" alt="Veronica Phillip, CPTC">

# Hi 👋, I'm Veronica Phillip, CPTC

### Documentation Systems & AI Implementation Specialist · Lead Technical Writer

*Documentation isn't about writing. It's about building a system.*

<a href="https://www.linkedin.com/in/veronicaphillip/"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"></a>
<a href="https://veronicaphillip.com/"><img src="https://img.shields.io/badge/Website-4D4DFF?style=for-the-badge&logo=google-chrome&logoColor=white" alt="Website"></a>
<a href="https://vikkivero.github.io/technical-writing-portfolio/"><img src="https://img.shields.io/badge/View_Portfolio-5E17EB?style=for-the-badge&logo=github&logoColor=white" alt="View Portfolio"></a>
<a href="https://substack.com/@veronicaphillip"><img src="https://img.shields.io/badge/Substack-FF6719?style=for-the-badge&logo=substack&logoColor=white" alt="Substack"></a>

<img src="https://komarev.com/ghpvc/?username=vikkivero&color=5E17EB&style=flat-square&label=Profile+Views" alt="Profile views">

</div>

## 👩🏽‍💻 About Me

I build governed documentation and knowledge systems that help SaaS teams, customers and AI tools find and use reliable information.

With more than 20 years of experience across IT, network technology and SaaS, I turn fragmented content into scalable documentation environments that support product delivery, customer self-service and AI-first operations.

## 🧩 What I Build

- **Docs-as-Code systems** using Markdown, Git, GitHub Actions, pull requests and automated publishing
- **Knowledge systems** designed for human users and AI workflows
- **API documentation, SDK guides and developer portals**
- **SaaS Help Centres, user guides and onboarding content**
- **SOPs, process documentation and operational playbooks**
- **Training materials, quick-reference guides and visual documentation**

## 📈 Selected Results

| Outcome | Result |
|---|---:|
| SaaS Help Centre adoption | **40% → 75%** |
| Monthly Help Centre engagement | **2,000 → 5,000 users** |
| Support tickets | **Reduced by 60%** |
| Tech startup product development cycle | **6 months → 3 months** |
| Documentation reach | **Teams and clients across 4 continents** |

## 💼 Featured Portfolio

### [▶ View the rendered portfolio](https://vikkivero.github.io/technical-writing-portfolio/)

### [📁 Explore the portfolio repository](https://github.com/vikkivero/technical-writing-portfolio)

| Portfolio Section | View Samples |
|---|---|
| Docs-as-Code | [Explore](https://github.com/vikkivero/technical-writing-portfolio/tree/main/Docs-as-Code) |
| SaaS User Guides | [Explore](https://github.com/vikkivero/technical-writing-portfolio/tree/main/SaaS-User-Guides) |
| Professional Reports | [Explore](https://github.com/vikkivero/technical-writing-portfolio/tree/main/Professional-Reports) |
| Instruction Design & Training | [Explore](https://github.com/vikkivero/technical-writing-portfolio/tree/main/Instruction-Design-and-Training) |
| Workbooks | [Explore](https://github.com/vikkivero/technical-writing-portfolio/tree/main/Workbooks) |
| Presentations & Infographics | [Explore](https://github.com/vikkivero/technical-writing-portfolio/tree/main/Presentations-and-Infographics) |

## 🛠️ Tools and Technologies

**Documentation and Publishing**

`Markdown` · `Quarto` · `HTML` · `CSS` · `DITA/XML` · `Oxygen XML Editor`

**Version Control and Automation**

`Git` · `GitHub` · `GitHub Actions`

**Knowledge and Process Systems**

`Confluence` · `CQL` · `Jira` · `JQL` · `SAP Signavio` · `Microsoft Visio`

**Development**

`Python`

## 📬 Connect With Me

- [LinkedIn](https://www.linkedin.com/in/veronicaphillip/)
- [VeronicaPhillip.com](https://veronicaphillip.com/)
- [ProTech Write & Edit Inc.](https://www.linkedin.com/company/11442435/)
- [Substack](https://substack.com/@veronicaphillip)

---

<div align="center">

*Building trusted knowledge systems for people, products and AI.*

</div>
